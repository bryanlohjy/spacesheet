...
export default class DataPicker {
  ...
  zoom(degree) {
    ...
    if (scale < this.minScale) {
      // if less than the min, reset view
      this.ctx.setTransform(1, 0, 0, 1, 0, 0);
    } else if (scale > this.maxScale) {
      // if greaterm then return
  ...
  isExceedingDataBounds(transform) {
    ...
    const rightFloat =
      this.viewportWidth / this.fittingScale -
      (translateX + this.width * scale);
    const bottomFloat =
      this.viewportHeight / this.fittingScale -
      (translateY + this.height * scale);
  ...
  get subdivisions() {
    // return the amount to subdivide by
  ...
  updateTransform() {
    // limit panning + update scale attribute
    let {
      a: scale,
      b,
      c,
...
class Cell {
  ...
  computeData() {
    ...
    if (this.subrow == 0 && this.subcolumn > 0) {
      ...
      const toAnchorKey = `${(this.column + 1) % this.dataPicker.columns}-${
        this.row
      }`;
    ...
    if (this.subrow > 0) {
      ...
      const toAnchorKey = `${this.subdivisions}-${this.column}-${
        (this.row + 1) % this.dataPicker.rows
      }-${this.subcolumn}-0`;
      ...
      if (!fromAnchor) {
        ...
        const cellParams = {
          ...
          get w() {
            return (
              self.dataPicker.width /
              (self.dataPicker.columns * self.subdivisions)
            );
          },
          get h() {
            return (
              self.dataPicker.height /
              (self.dataPicker.rows * self.subdivisions)
            );
          },
          drawFn: self.drawFn,
